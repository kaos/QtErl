...
#include <QtCore/5.0.1/QtCore/private/qmetaobject_moc_p.h>
...
bool Owc::parseClassHead(ClassDef *def)
{
...
bool Owc::parseEnum(EnumDef *def)
{
...
void Owc::parseFunctionArguments(FunctionDef *def)
{
    Q_UNUSED(def);
    while (hasNext()) {
        ArgumentDef  arg;
...
bool Owc::testFunctionAttribute(FunctionDef *def)
{
...
bool Owc::testFunctionAttribute(Token tok, FunctionDef *def)
{
    switch (tok) {
        case Q_MOC_COMPAT_TOKEN:
            def->isCompat = true;
...
bool Owc::testFunctionRevision(FunctionDef *def)
{
...
bool Owc::parseFunction(FunctionDef *def, bool inMacro)
{
    def->isVirtual = false;
    def->isStatic = false;
...
bool Owc::parseMaybeFunction(const ClassDef *cdef, FunctionDef *def)
{
    def->isVirtual = false;
    def->isStatic = false;
    //skip modifiers and attributes
...
void Owc::generate(FILE *out)
{
    ...
    fprintf(out,
            "/****************************************************************************\n"
            "** Object wrapper code from reading C++ file '%s'\n**\n" , fn.constData());
    fprintf(out, "** Created by: The Object Wrapper Compiler version %d (Qt %s)\n**\n" , mocOutputRevision, QT_VERSION_STR);
    ...
    fprintf(out, "#error \"This file was generated using the owc built with Qt %s."
...
void Owc::parseSlots(ClassDef *def, FunctionDef::Access access)
{
...
void Owc::parseSignals(ClassDef *def)
{
    int defaultRevision = -1;
    if (test(Q_REVISION_TOKEN)) {
        next(LPAREN);
...
void Owc::createPropertyDef(PropertyDef &propDef)
{
...
void Owc::parseProperty(ClassDef *def)
{
    next(LPAREN);
    PropertyDef propDef;
...
void Owc::parsePluginData(ClassDef *def)
{
    next(LPAREN);
    QByteArray metaData;
    while (test(IDENTIFIER)) {
...
void Owc::parsePrivateProperty(ClassDef *def)
{
    next(LPAREN);
    PropertyDef propDef;
    next(IDENTIFIER);
...
void Owc::parseEnumOrFlag(ClassDef *def, bool isFlag)
{
    next(LPAREN);
...
void Owc::parseFlag(ClassDef *def)
{
    next(LPAREN);
...
void Owc::parseClassInfo(ClassDef *def)
{
    next(LPAREN);
...
void Owc::parseInterfaces(ClassDef *def)
{
    next(LPAREN);
    while (test(IDENTIFIER)) {
...
void Owc::parseDeclareInterface()
{
    next(LPAREN);
...
void Owc::parseSlotInPrivate(ClassDef *def, FunctionDef::Access access)
{
    next(LPAREN);
    FunctionDef funcDef;
    next(IDENTIFIER);
...
QByteArray Owc::lexemUntil(Token target)
{
    int from = index;
    until(target);
    QByteArray s;
...
void Owc::checkSuperClasses(ClassDef *def)
{
...
void Owc::checkProperties(ClassDef *cdef)
{
    //
